/**
 * The handful of icons the window uses, inlined so nothing is fetched and
 * nothing depends on an icon font. Outlines from Lucide (ISC licence),
 * 24-unit grid, drawn with currentColor.
 */
...
function icon(paths: string) {
  return function Icon({ size = 18, ...rest }: Props) {
    return (
      <svg
        width={size}
        height={size}
...
export const Check = icon('<circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/>');
...
export const CircleCheck = icon('<circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/>');
